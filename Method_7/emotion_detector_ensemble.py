...
class EnsembleEmotionDetector:
    ...
    def train(self, dataset_path):
        try:
            ...
            model_path = os.path.join(os.path.dirname(__file__), 'emotion_model_ensemble.joblib')
    ...
    def predict_emotion(self, text):
        ...
        try:
            ...
            class_probs = {
                'Non Emotional': float(probas[0]),
                'Emotional': float(probas[1])
            }
...
class EnsembleEmotionDetectorApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.detector = EnsembleEmotionDetector()
        self.title("Emotion Detection (Ensemble)")
        ...
        self.title_label = ctk.CTkLabel(
            self.container, 
            text="Cricket Emotion Detection using Ensemble Model",
        ...
        self.probabilities_label = ctk.CTkLabel(
            self.result_frame,
            text="",
            font=("Helvetica", 12),
            wraplength=800,
        ...
        self.probabilities_label.pack(pady=5, fill=tk.X)
    ...
    def analyze_text(self):
        ...
        try:
            emotion, confidence, class_probs = self.detector.predict_emotion(text)
            ...
            if class_probs:
                ...
                prob_text += ", ".join([f"{k}: {v*100:.1f}%" for k, v in class_probs.items()])
                self.probabilities_label.configure(text=prob_text)
...
if __name__ == "__main__":
    try:
        ...
        app = EnsembleEmotionDetectorApp()
        app.mainloop()
    except Exception as e:
